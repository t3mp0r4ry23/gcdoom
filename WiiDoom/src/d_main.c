...
static void IdentifyVersion (void)
{
	...
	{
		if (selectedIWADFile[i] == '/')
		{
			i++;
			while (selectedIWADFile[i] != '.' && i < strlen(selectedIWADFile))
			{
				tempIWADName[ii++] = selectedIWADFile[i++];
			}
			tempIWADName[ii] = '\0';
			break;
		}
...
void WADPicker()
{
	...
	int LOGOY = 10;
	int CHAR_YPOS;
	char cfgLocations[NumCFGLocations][80];
	bool useMouse = 0;
	Uint32 joyWait = SDL_GetTicks();
	int i;
	...
	SDL_Rect startTextRect = {STARTX + 60, STARTY + 5, STARTWIDTH, STARTHEIGHT};
	...
	if (dir != NULL)
	{
		// Get a count of PWADs and a list of their names
		dirent = readdir(dir);
		...
		while (dirent != NULL)
		{
			...
			{
				// note that numPWADSFound is used before incrementing 
				//  because the array index starts at zero
				strncpy(foundPwads[numPWADSFound], dirent->d_name, len-4);
		...
		// Reset dir (no longer needed because count and name loops were combined)
		//		rewinddir(dir);
	}
	closedir(dir);
	bool done = false;
	...
	while (!done)
	{
		...
		while (SDL_PollEvent(&Event))
		{
			...
			case SDL_KEYDOWN:
				...
				else if (Event.key.keysym.sym == SDLK_RIGHT || Event.key.keysym.sym == SDLK_a)
				{
					input = D_RIGHT;
				}
		...
		// Display PWAD Header
		SDL_Surface *pwadHeader;
		if (menuSelected != 0)
			pwadHeader = TTF_RenderText_Solid(doomfnt18, "PWAD (Optional)" , clrFgSelected);
		else
			pwadHeader = TTF_RenderText_Solid(doomfnt18, "PWAD (Optional)" , clrStartText);		
		SDL_BlitSurface(pwadHeader, NULL, screen, &pwadHeaderRect);
		SDL_FreeSurface(pwadHeader);
		...
		for (x=0; (((PWADSTARTNUM + x) < numPWADSFound) && (x < PWADMAXPAGE)); x++)
		{
			...
			for (selectedPWADIndex = 0; selectedPWADIndex < MAX_PWADS; selectedPWADIndex++)
			{
				if (selectedPWADs[selectedPWADIndex] == i)
				{
					found = true;
					if (i == PWADMenu)
						sText = TTF_RenderText_Solid(doomfnt24, foundPwads[i] , clrFgBoth);
					else
			...
			{
				if (PWADMenu == x)
					sText =  TTF_RenderText_Solid(doomfnt24, foundPwads[i] , clrFgLit);
				else
		...
		*/
		...
		for (i=0; i<numIWADSFound; i++)
		{
			if ((ax > IWADBOXX) && (ax < (IWADBOXX + IWADBOXWIDTH)) && 
				(ay > CHAR_YPOS-(IWADCHARSPACING/2)) && (ay < (CHAR_YPOS + (IWADCHARSPACING/2))) && 
				input == D_SELECT)
			{
				selectedIWAD = i;
				menuSelected = 0;
		...
		for (x=0; ((x < numPWADSFound) && (x < PWADMAXPAGE)); x++)
		{
			...
			if ((ax > PWADBOXX) && (ax < (PWADBOXX + PWADBOXWIDTH)) && 
				(ay > CHAR_YPOS-(PWADCHARSPACING/2)) && (ay < (CHAR_YPOS + (PWADCHARSPACING/2))) && 
				input == D_SELECT &&
				(SDL_GetTicks() > joyWait)
				|| (PWADMenu == i &&  input == D_SELECT &&
				(SDL_GetTicks() > joyWait)))
			{
				bool deselected = false;

				// Need to deselect?
				for (selectedPWADIndex = 0; selectedPWADIndex < numPWADSFound; selectedPWADIndex++)
				{
					if (selectedPWADs[selectedPWADIndex] == i)
					{
		...
		if ((ax > STARTX) && (ax < STARTX+STARTWIDTH) && (ay >= STARTY-STARTHEIGHT) && 
			(selectedIWAD != -1) && input == D_SELECT)
		{
			done = true;
		}
		else if ((selectedIWAD != -1 && menuSelected == 0) && input == D_SELECT)
		{
			menuSelected = 1;
		}
		else if (menuSelected == 1 && input == D_BACK)
		{
			menuSelected = 0;
		}
		if (input == D_DOWN && (SDL_GetTicks() > joyWait))
		{
			joyWait = SDL_GetTicks() + 20;
			if (menuSelected == 0)
			{
				selectedIWAD++;
				if (selectedIWAD >= numIWADSFound)
					selectedIWAD = 0;
			}
			else if (menuSelected == 1)
			{
				PWADMenu++;
				if (PWADMenu >= numPWADSFound)
					PWADMenu = 0;
			}
		}
		if (input == D_UP && (SDL_GetTicks() > joyWait))
		{
			joyWait = SDL_GetTicks() + 20;
			if (menuSelected == 0)
			{
				selectedIWAD--;
				if (selectedIWAD < 0)
					selectedIWAD = numIWADSFound-1;
			}
			else if (menuSelected == 1)
			{
				PWADMenu--;
				if (PWADMenu < 0)
					PWADMenu = numPWADSFound-1;
			}
		}
		if (input == D_LEFT && (SDL_GetTicks() > joyWait) && (PWADSTARTNUM > 0)) 
		{
		...
		if (input == D_RIGHT && (SDL_GetTicks() > joyWait) && (PWADSTARTNUM + PWADMAXPAGE < numPWADSFound))
		{
			PWADSTARTNUM += PWADMAXPAGE;
			joyWait = SDL_GetTicks() + 20;
		}
	...
	char loadingText[80];
	sprintf(loadingText,"Loading %s...", foundIwads[selectedIWAD]);
	// Blank screen and start Doom
	SDL_Surface *loadingSurface = TTF_RenderText_Solid(doomfnt24, loadingText, clrFgSelected);
	SDL_Rect loadingRect = 
	{
		getCenter(640,loadingSurface->w),
		getCenter(480,loadingSurface->h),
		loadingSurface->w,
		loadingSurface->h,
	};
	SDL_FillRect(screen, NULL, SDL_MapRGB(screen->format, 0, 0, 0));
