...
setup_menu_t keys_settings1[] =  // Key Binding screen strings
{
  ...
  {"EXIT"        ,S_KEY       ,m_menu,KB_X,KB_Y+19*8,{&key_escape}},
...
setup_menu_t cred_settings[]={
  ...
  {"David Hudson",S_SKIP|S_CREDIT|S_LEFTJUST,m_null, CR_X2, CR_Y + CR_S*(prog+1) + CR_SH*cr_prog},
...
bool M_Responder (event_t* ev) {
      ...
      if (ch == key_menu_enter)
  {
    ...
    return true;
      ...
      if ((ch == key_escape) || (ch == key_menu_backspace))
  {
  ...
  if (ch == key_escape)                           // phares 3/7/98
    {
      currentMenu->lastOn = itemOn;
      M_ClearMenus ();
      S_StartSound(NULL,sfx_swtchx);
